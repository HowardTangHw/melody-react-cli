...
function createApp(name, verbose, version, useNpm, usePnp, useTypescript, useMobx, template) {
  ...
  run(root, appName, version, verbose, originalDirectory, template, useYarn, usePnp, useTypescript, useMobx);
}
...
function run(root, appName, version, verbose, originalDirectory, template, useYarn, usePnp, useTypescript, useMobx) {
  getInstallPackage(version, originalDirectory).then(packageToInstall => {
    ...
    if (useMobx) {
      allDependencies.push('mobx', 'mobx-react');
    }
    ...
    getPackageName(packageToInstall)
      ...
      .then(info => {
        ...
        return install(root, useYarn, usePnp, allDependencies, verbose, isOnline).then(() => packageName);
      })
